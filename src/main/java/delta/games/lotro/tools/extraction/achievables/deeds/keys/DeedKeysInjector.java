package delta.games.lotro.tools.extraction.achievables.deeds.keys;
  ...
  private DeedDescription handleClassDeed(DeedDescription deed)
  {
    ...
    if (name.startsWith("Class Deeds"))
    {
      ...
      if ((index!=-1) && (index2!=-1))
      {
        ...
        {
          AbstractClassDescription requiredClass=candidateDeed.getUsageRequirement().getRequiredClass();
  ...
  private DeedDescription handleRaceSuffix(DeedDescription deed)
  {
    ...
    for(String race : RACES)
    {
      ...
      if (name.endsWith(suffix))
      {
        ...
        for(DeedDescription candidateDeed : candidateDeeds)
        {
          RaceDescription requiredRace=candidateDeed.getUsageRequirement().getRequiredRace();
          if ((requiredRace!=null) && (requiredRace.getName().equals(fixedRace)))
          {
            return candidateDeed;
          }
        }
